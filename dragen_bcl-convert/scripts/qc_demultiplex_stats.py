...
def main():
    ...
    # PNG output using Plotly has a dependency on Google Chrome!
    #
    logging.info(f"Creating bar charts")
    workdir = os.path.dirname(os.path.abspath(args.file))
    os.chdir(workdir)
    plot_seaborn_bar(df_demux_stats, threshold=args.threshold) # to png file
    plot_plotly_bar(df_demux_stats, threshold=args.threshold)  # to html file
    # plot_plotly_box(df_demux_stats, threshold=args.threshold)
